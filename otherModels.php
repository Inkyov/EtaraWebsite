...
if($num>0){
      ...
      echo "<div id='details'>";
            ...
            echo "<p id='bigFont'>{$Name}</p><br/>";
            echo "<p><span class='info1'>Описание: </span><span class='info2' id='beige'>{$Description}</span></p>";
            echo "<p><span class='info1'>Размер: </span><span class='info2'><select id='selectSize{$ProductID}' class='select{$ProductID}'> 
                  <option value='1x3'>1x3см</option>
                  <option value='2x6'>2x6см</option>
                  <option value='3x9'>3x9см</option>
                  <option value='4x12'>4x12см</option>
                  <option value='5x20'>5x20см</option>
                  </select></span>
                  </p>";
            echo "<p><span class='info1'>Вид материал: </span><span class='info2'><select id='selectMaterial{$ProductID}' class='select{$ProductID}'> 
                  <option value='yareshka'>Ярешка кожа</option>
                  <option value='teleshka'>Телешка кожа</option>
                  <option value='agneshka'>Агнешка кожа</option>
                  </select></span>
                  </p>";
            echo "<p><span class='info1'>Цвят: </span><span class='info2' id='beige'>{$Color}</span></p>";
            echo "<p>";
                echo "<span class='info1'>Цена: </span><span class='info2'><p class='price{$ProductID}' id='beige'>" . number_format($Price, 2, '.' , ',') . "лв. </p></span>";
            echo "</p>";
